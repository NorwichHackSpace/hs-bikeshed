...
function md5Hash(str: string): string {
  // Simple MD5 — we generate it at render time for a Gravatar URL,
  // so we use the Web Crypto API isn't available synchronously.
  // Instead, use a minimal MD5 implementation.
...
export function UserAvatar({ size = 34, sx, ...props }: UserAvatarProps) {
  ...
  // Priority: Uploaded avatar → Gravatar → Google → initials
  let src: string | undefined
  if (profileAvatarUrl && !profileAvatarFailed) {
    src = profileAvatarUrl
  } else if (email && !gravatarFailed) {
